...
def request(req):
    ...
    if req.straight_line != 0.0 and req.turn_angle == 0.0:
        ...
        while xt < abs(req.straight_line):
            ...
            modification = odometry_value.twist.twist.angular.z
            #rospy.loginfo("%s",modification)

            if abs(speed.linear.x) >0.01 and 0.05 > abs(speed.linear.x):
                if speed.linear.x >0 and modification < 0.0:
                    speed.angular.z = modification
                elif speed.linear.x >0 and modification > 0.0:
                    speed.angular.z = -modification
                elif speed.linear.x <0 and modification < 0.0:
                    speed.angular.z = -modification
                else:
                ...
                speed.angular.z =0.0
            #rospy.loginfo("x= %s",speed.linear.x)
            #rospy.loginfo("z= %s",speed.angular.z)
    ...
    # 回転運動
        ...
        while xt < abs(order_value):
            ...
            modification = odometry_value.twist.twist.linear.x
            #rospy.loginfo("%s",modification)

            if abs(speed.angular.z) >0.01 and 0.05 > abs(speed.angular.z):
                if modification < 0.0:
                    speed.linear.x = modification
