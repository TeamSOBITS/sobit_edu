...
class JointController:
	def __init__(self):
		self.pub_arm_control = rospy.Publisher("/arm_trajectory_controller/command",  trajectory_msgs.msg.JointTrajectory, queue_size=10)
		self.pub_xtion_control = rospy.Publisher("/xtion_trajectory_controller/command", trajectory_msgs.msg.JointTrajectory, queue_size=10)
		self.servise = rospy.Service("gripper_open_and_close", sobit_common_msg.srv.gripper_ctrl, self.open_and_close_gripper_server)
		self.servise = rospy.Service("gripper_move_to_target", sobit_common_msg.srv.gripper_move, self.move_gripper_to_target_server)
		self.servise = rospy.Service("motion_ctrl", sobit_common_msg.srv.robot_motion, self.move_to_registered_motion_server)
	...
	def move_gripper_to_target_server(self, req_msg):
		...
		if key == False:
			...
			return sobit_common_msg.srv.gripper_moveResponse(False)
		...
		if object_z_cm < self.from_base_to_arm_flex_link_z_cm + self.arm_flex_link_z_cm:
		...
		self.move_arm_joint("arm_flex_joint"  , 0.0                 , time_from_start)
		rospy.sleep(time_from_start)
		...
		return sobit_common_msg.srv.gripper_moveResponse(True)
	...
	def open_and_close_gripper_server(self, req_msg):
		...
		rospy.sleep(time_from_start)
		return sobit_common_msg.srv.gripper_ctrlResponse(True)
	...
	def move_wheel(self, str_distance):
		...
		try:
			wheel_ctrl_service= rospy.ServiceProxy('/robot_ctrl/odom_base_ctrl', sobit_common_msg.srv.odom_base)
