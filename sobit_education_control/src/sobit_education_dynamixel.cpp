...
{
  /*
  dxl_goal_position[1] = toBit(1, 0);
  dxl_goal_position[16] = toBit(16, 0);
  dxl_goal_position[17] = toBit(17, 0);
  dxl_goal_position[18] = toBit(18, 1.31);
  dxl_goal_position[19] = toBit(19, 0);
  dxl_goal_position[20] = toBit(20, 0);*/
...
{
  if      (id == 16)  { return MAGNIFICATION_VALUE * (rad + M_PI);  }
  else if (id == 17)  { return MAGNIFICATION_VALUE * (-rad + M_PI); }
  else if (id == 18)  { return MAGNIFICATION_VALUE * (rad + M_PI);  }
  else if (id == 19)  { return MAGNIFICATION_VALUE * (rad + M_PI);  }
  else if (id == 20)  { return 2080 - rad * ( 910 / (M_PI_2) ); }
  else if (id == 1)   { return MAGNIFICATION_VALUE * (rad + M_PI); }
  return -1;
}

float SobitEducationDynamixel::toRad(std::string joint_name, int bit)
{
  if (joint_name == "arm_roll_joint")         { return ( bit / MAGNIFICATION_VALUE) - M_PI;  }
  else if (joint_name == "arm_flex_joint")    { return - ( bit / MAGNIFICATION_VALUE) + M_PI; }
  else if (joint_name == "elbow_flex_joint")  { return ( bit / MAGNIFICATION_VALUE) - M_PI; }
  else if (joint_name == "wrist_flex_joint")  { return ( bit / MAGNIFICATION_VALUE) - M_PI; }
  else if (joint_name == "hand_motor_joint")  { return - ( bit - 2080 ) / ( 910 / (M_PI_2) );  }
  else if (joint_name == "xtion_tilt_joint")  { return ( bit / MAGNIFICATION_VALUE) - M_PI;  }
}