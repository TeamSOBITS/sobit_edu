...
SobitEducationControl::SobitEducationControl()
{
  ...
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_arm_roll  (pos_handle_arm_roll,   joint_limits, soft_joint_limits);
  getJointLimits("arm_flex_joint",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_arm_flex  (pos_handle_arm_flex,   joint_limits, soft_joint_limits);
  getJointLimits("elbow_flex_joint",   nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_elbow_flex(pos_handle_elbow_flex, joint_limits, soft_joint_limits);
  getJointLimits("wrist_flex_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_wrist_flex(pos_handle_wrist_flex, joint_limits, soft_joint_limits);
  getJointLimits("hand_motor_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_hand_motor(pos_handle_hand_motor, joint_limits, soft_joint_limits);
  getJointLimits("xtion_tilt_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle   joint_limit_xtion_tilt(pos_handle_xtion_tilt, joint_limits, soft_joint_limits);
...
void SobitEducationControl::read(ros::Time time, ros::Duration period)
{
  ...
  {
    if(     pose.name[i] == "arm_roll_joint")         {pos_[0] = pose.position[i]; }
    else if(pose.name[i] == "arm_flex_joint")         {pos_[1] = pose.position[i]; }
    else if(pose.name[i] == "elbow_flex_joint")         {pos_[2] = pose.position[i]; }
    else if(pose.name[i] == "wrist_flex_joint")         {pos_[3] = pose.position[i]; }
    else if(pose.name[i] == "hand_motor_joint")         {pos_[4] = pose.position[i]; }
    else if(pose.name[i] == "xtion_tilt_joint")         {pos_[5] = pose.position[i]; }
  }
}
