...
SobitEducationControl::SobitEducationControl() {
  ...
  hardware_interface::JointStateHandle state_handle_xtion_pan("xtion_pan_joint", &pos_[6], &vel_[6], &eff_[6]);
  ...
  jnt_state_interface_.registerHandle(state_handle_xtion_pan);
  ...
  hardware_interface::JointHandle pos_handle_xtion_pan(jnt_state_interface_.getHandle("xtion_pan_joint"), &cmd_[6]);
  ...
  jnt_pos_interface_.registerHandle(pos_handle_xtion_pan);
  ...
  getJointLimits("xtion_pan_joint", nh_, joint_limits);
  joint_limits_interface::PositionJointSoftLimitsHandle joint_limit_xtion_pan(
      pos_handle_xtion_pan, joint_limits, soft_joint_limits);
  ...
  jnt_limit_interface_.registerHandle(joint_limit_xtion_pan);
...
void SobitEducationControl::read(ros::Time time, ros::Duration period) {
  ...
  for (int i = 0; i < pose.name.size(); i++) {
    ...
    } else if (pose.name[i] == "xtion_pan_joint") {
...
void SobitEducationControl::write(ros::Time time, ros::Duration period) {
  ...
  traj.joint_names.push_back("xtion_pan_joint");
