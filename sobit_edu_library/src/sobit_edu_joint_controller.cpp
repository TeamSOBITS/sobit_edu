...
bool JointController::moveToPose(
  ...
  // Move to the pose
  if (is_pose) {
      RCLCPP_INFO(this->get_logger(), "[SOBIT EDU] Pose '%s' was found.", pose_name.c_str());
      ...
      RCLCPP_WARN(this->get_logger(), "[SOBIT EDU] Pose '%s' was not found.", pose_name.c_str());
      return false;
  }
}
...
bool JointController::moveAllJointsRad(
  ...
  try {
    ...
    RCLCPP_ERROR(this->get_logger(), "[SOBIT EDU] %s", ex.what());
...
bool JointController::moveHandToTargetCoord(
    ...
    const int32_t sec , bool is_sleep ) {
  // sobit_edu::WheelController wheel_ctrl;
...
// RCLCPP_COMPONENTS_REGISTER_NODE(sobit_edu::JointController)